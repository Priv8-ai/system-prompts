...
export const createAIModelAdapter = (provider: AIProvider) => {
  switch (provider.id) {
    case 'ollama':
...
abstract class BaseAIAdapter {
  ...
  abstract generateResponse(modelId: string, messages: ChatMessage[], systemPrompt?: string | null, files?: FileAttachment[] | null): Promise<string>;
}
...
class OllamaAdapter extends BaseAIAdapter {
  ...
  async generateResponse(modelId: string, messages: ChatMessage[], systemPrompt: string | null = null, files: FileAttachment[] | null = null): Promise<string> {
