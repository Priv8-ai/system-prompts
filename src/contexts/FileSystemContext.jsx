...
export const FileSystemProvider = ({ children }) => {
  ...
  const getItemsInDirectory = (dirPath) => {
  ...
  const createFile = (dirPath, name, initialContent = '') => {
  ...
  const deleteItem = (path, isDirectory = false) => {
    if (isDirectory) {
  ...
  const renameItem = (oldPath, newPath, isDirectory = false) => {
    if (isDirectory) {
  ...
  const importProjectFromZip = async (zipData) => {
